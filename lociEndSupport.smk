if GENOMETOPREVIOUS is not None:
    ...
    rule mergePreviousPhaseTmsWithGencode:
        input:
            previous=lambda wildcards: GENOMETOPREVIOUS[
...
rule mergePreviousPhaseTmsWithGencodeBiotypes:
    ...
    output:
        "mappings/nonAnchoredMergeReads/previous+biotypes/{capDesign}.previous.tmerge.all.gff.gz",
    shell:
        """
uuid=$(uuidgen)
uuidTmpOut=$(uuidgen)
...
if GENOMETOPREVIOUS is not None:
    ...
    rule mergeCurrentPreviousPhaseTmsWithGencode:
        input:
            current=lambda wildcards: expand(
                "mappings/nonAnchoredMergeReads/{techname}Corr{corrLevel}_{capDesign}_{sizeFrac}_{barcodes}.HiSS.tmerge.min{minReadSupport}reads.splicing_status:all.endSupport:all.gff.gz",
                filtered_product_merge,
                techname=wildcards.techname,
                corrLevel=wildcards.corrLevel,
                capDesign=wildcards.capDesign,
                sizeFrac=wildcards.sizeFrac,
                barcodes=wildcards.barcodes,
...
rule mergeCurrentPreviousPhaseTmsWithGencodeBiotypes:
    input:
        clsGencode="mappings/nonAnchoredMergeReads/mergeWithPrevious/{techname}Corr{corrLevel}_{capDesign}_{sizeFrac}_{barcodes}.HiSS.tmerge.min{minReadSupport}reads.endSupport:all.gff.gz",
        gencode="annotations/simplified/{capDesign}.gencode.collapsed.simplified_biotypes.gtf",
    output:
        "mappings/nonAnchoredMergeReads/mergeWithPrevious+biotypes/{techname}Corr{corrLevel}_{capDesign}_{sizeFrac}_{barcodes}.HiSS.tmerge.min{minReadSupport}reads.endSupport:all.gff.gz",
    shell:
        """
uuid=$(uuidgen)
uuidTmpOut=$(uuidgen)
...
rule getGencodeSupportedEnds:
    input:
        ...
        cagePeaks=lambda wildcards: GENOMETOCAGEPEAKS[
            CAPDESIGNTOGENOME[wildcards.capDesign]
        ],
...
rule getFlLocusStats:
    ...
    output:
        config["STATSDATADIR"]
        + "tmp/{techname}Corr{corrLevel}_{capDesign}_{sizeFrac}_{barcodes}.min{minReadSupport}reads.FLloci.stats.tsv",
    shell:
        r"""
#PCG stats
...
rule aggFlLocusStats:
    input:
        lambda wildcards: expand(
            ...
            corrLevel=FINALCORRECTIONLEVELS,
    ...
    output:
        config["STATSDATADIR"] + "all.min{minReadSupport}reads.FLloci.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
