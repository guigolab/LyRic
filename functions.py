...
def getMergedSampleReps(wildcards):
    ...
    #######################################################################
    inputFiles = []
    for basename in (sampleRepGroupIdToSampleReps[wildcards.groupedSampleRepBasename]):
        inputFiles.append("output/mappings/mergedReads/" + basename +
                          ".HiSS.tmerge.splicing_status-all.endSupport-all.gff.gz")
    return(inputFiles)
