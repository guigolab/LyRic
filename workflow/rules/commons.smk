...
def filtered_product(*args):
    ...
    if len(args) < 2:
        quit(
            "Error in function filtered_product (wrong number of arguments, shoud be >1)."
        )
    elif len(args) == 2:
...
def multi_figures(capDesign, sizeFrac, sampleRep, techname, splicing_status=None):
    ...
    figure_settings["hideXaxisLabels"] = (
        "theme(axis.ticks.x = element_blank(), axis.text.x = element_blank()) +"
    )
    removeFacetLabels = ""
    if (
        techname not in ("byTech")
        and capDesign not in ("byCapDesign")
        and sampleRep not in ("bySampleRep")
    ):
        removeFacetLabels = """
...
"""
    ...
    if splicing_status is not None:
    ...
    if techname not in ("byTech"):
        figure_settings["technameFilterString"] = (
            "dat <- subset(dat, seqTech=='" + techname + "')\n"
        )
    figure_settings["substSeqTechString"] = r"""
dat$seqTech <- gsub('-', '\n', dat$seqTech)
"""
    figure_settings["substSampleRepString"] = (
        "dat$sampleRep <- gsub('" + capDesign + "_', '', dat$sampleRep)\n"
    )
...
"""
    ...
    figure_settings["facetPlotSetup"] = """
...
""".format(removeFacetLabels)

    if capDesign not in ("byCapDesign"):
        figure_settings["capDesignFilterString"] += (
            "dat <- subset(dat, capDesign=='" + capDesign + "')\n"
        )

    figure_settings["sizeFracFilterString"] += (
        "dat <- subset(dat, sizeFrac=='" + sizeFrac + "')\n"
    )
    if sampleRep not in ("bySampleRep"):
        figure_settings["sampleRepFilterString"] += (
            "dat <- subset(dat, sampleRep=='" + sampleRep + "')\n"
        )
    ...
    return (
        returnSubGroup1String
        + " "
        + " ".join(sorted(returnSubGroup1StringData))
        + "\n"
        + returnSubGroup2String
        + " "
        + " ".join(sorted(returnSubGroup2StringData))
        + "\n"
        + returnSubGroup3String
        + " "
        + " ".join(sorted(returnSubGroup3StringData))
    )
...
def getMergedSampleReps(wildcards):
    ...
    for comb in itertools.product(
        sampleRepGroupIdToSampleReps[wildcards.groupedSampleRepBasename],
        wildcards.minReadSupport,
    ):
        inputFiles.append(
            "output/mappings/mergedReads/"
            + comb[0]
            + ".HiSS.tmerge.min"
            + comb[1]
            + "reads.splicing_status-all.endSupport-all.gff.gz"
        )
    return inputFiles
