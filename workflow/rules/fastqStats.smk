...
rule fastqTimestamps:
    ...
    output:
        "output/statsFiles/" + "all.fastq.timestamps.tsv",
    shell:
        r"""
uuid=$(uuidgen)
...
zcat {input} | fastq2tsv.pl | perl -F"\t" -slane '$F[0]=~s/^(\S+).*/$1/; print join("\t", @F)' | awk -v t={wildcards.techname} -v c={wildcards.capDesign} -v si={wildcards.sizeFrac} -v b={params.bc} '{{print t,c,si,b,length($2)}}' OFS="\t" | gzip >> {TMPDIR}/$uuidTmpOut.gz
