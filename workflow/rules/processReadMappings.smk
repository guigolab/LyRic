...
rule strandGffs:
    input:
        ...
        strandInfo="output/mappings/integratePolyaAndSjInfo/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.polyA+SJ.strandInfo.tsv",
    output:
        "output/mappings/strandGffs/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.stranded.gff.gz",
    shell:
        """
uuid=$(uuidgen)
uuidTmpOut=$(uuidgen)
...
rule aggIntraPrimingStats:
    ...
    output:
        "output/statsFiles/" + "all.intraPriming.stats.tsv",
    shell:
        """
uuid=$(uuidgen)
...
rule highConfidenceReads:
    ...
    params:
        minSJsPhredScore=lambda wildcards: sampleAnnotDict[
            wildcards.techname
            + "_"
            + wildcards.capDesign
            + "_"
    ...
    output:
        ...
        stats="output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.highConfSplicedReads.stats.tsv",
    shell:
        r"""
uuidTmpOutG=$(uuidgen)
uuidTmpOutS=$(uuidgen)
...
rule aggHighConfSplicedReadsStats:
    ...
    output:
        "output/statsFiles/" + "all.highConfSplicedReads.stats.tsv",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getHCGMintrons:
    input:
        "output/mappings/highConfidenceReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.strandedHCGMs.gff.gz",
    output:
        temp(
            "output/mappings/highConfidenceReads/introns/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.strandedHCGMs.introns.tsv"
        ),
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getHiSeqSupportedHCGMs:
    ...
    output:
        "output/mappings/highConfidenceReads/HiSS/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.gff.gz",
    shell:
        """
...
rule aggHiSSStats:
    ...
    output:
        "output/statsFiles/" + "all.HiSS.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule mergedReads:
    ...
    output:
        temp(
            "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.endSupport-all.gff"
        ),
    threads: 1
...
rule mergedUnfilteredSirvReads:
    input:
        "output/mappings/strandGffs/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.stranded.gff.gz",
    output:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.noFilt.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-all.gff.gz",
    threads: 1
    wildcard_constraints:
        sizeFrac=r"[0-9-+\.]+",
    shell:
...
rule splitTmsBySplicedStatus:
    input:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.endSupport-all.gff",
    output:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.splicing_status-{splicedStatus}.endSupport-all.gff.gz",
    params:
        grepSpliced=lambda wildcards: (
            "| tgrep -F 'spliced \"1\"'"
            if wildcards.splicedStatus == "spliced"
            else (
                "| tgrep -F 'spliced \"0\"'"
                if wildcards.splicedStatus == "unspliced"
                else ""
            )
        ),
    shell:
...
rule catMergedReadsForGroupedSampleReps:
    ...
    output:
        gff=temp(
            "output/mappings/mergedReads/groupedSampleReps/tmp/{groupedSampleRepBasename}.min{minReadSupport}reads.splicing_status-all.endSupport-all.gff"
        ),
        countFiles=temp(
            "output/mappings/mergedReads/groupedSampleReps/tmp/{groupedSampleRepBasename}.min{minReadSupport}reads.splicing_status-all.endSupport-all.countFiles.txt"
        ),
    shell:
        """
uuid=$(uuidgen)
...
rule mergedReadsToBed:
    ...
    output:
        temp(
            "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-all.bed"
        ),
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getTmStats:
    input:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.HiSS.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-{endSupport}.gff.gz",
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.splicing_status-all.endSupport-{endSupport}.TmStats.stats.tsv.gz",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule aggTmStats:
    ...
    output:
        "output/statsFiles/"
        + "all.{capDesign}.min{minReadSupport}.endSupport-{endSupport}.TmStats.stats.tsv.gz",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule getMergingStats:
    ...
    output:
        "output/statsFiles/"
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.merged.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule aggMergingStats:
    ...
    output:
        "output/statsFiles/" + "all.min{minReadSupport}reads.merged.stats.tsv",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
echo -e "seqTech\tcapDesign\tsizeFrac\tsampleRep\tcategory\tcount" > {TMPDIR}/$uuidTmpOut
...
rule aggTmLengthStats:
    input:
        all=lambda wildcards: expand(
            "output/statsFiles/"
            + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.splicing_status-all.endSupport-all.TmStats.stats.tsv.gz",
            filtered_product,
            techname=TECHNAMES,
            capDesign=CAPDESIGNS,
            sizeFrac=SIZEFRACS,
        ...
        fl=lambda wildcards: expand(
            "output/statsFiles/"
            + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.splicing_status-all.endSupport-cagePolyASupported.TmStats.stats.tsv.gz",
            filtered_product,
            techname=TECHNAMES,
            capDesign=CAPDESIGNS,
            sizeFrac=SIZEFRACS,
    ...
    output:
        "output/statsFiles/"
        + "all.min{minReadSupport}reads.matureRNALength.stats.tsv.gz",
    shell:
        r"""
uuidTmpOut=$(uuidgen)
...
rule aggGeneReadCoverageStats:
    input:
        lambda wildcards: expand(
            "output/mappings/geneReadCoverage/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.coverage.tsv",
            filtered_product,
            techname=TECHNAMES,
            capDesign=CAPDESIGNS,
            sizeFrac=SIZEFRACS,
    ...
    output:
        "output/statsFiles/"
        + "all.tmerge.min{minReadSupport}reads.GeneReadCoverage.stats.tsv",
    shell:
        r"""
uuid=$(uuidgen)
