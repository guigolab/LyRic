...
rule getReadsSpliceSites:
	...
	output: "mappings/" + "makeIntrons/readSpliceSites/{techname}_{capDesign}.introns.{spliceType}.tsv.gz"
	shell:
		'''
...
rule getTmSpliceSites:
	...
	output: "mappings/" + "nonAnchoredMergeReads/pooled/spliceSites/{techname}_{capDesign}.introns.{spliceType}.tsv.gz"
	shell:
		'''
...
rule getGeneidScores:
	input:
		...
		rawReads="mappings/" + "makeIntrons/readSpliceSites/{techname}_{capDesign}.introns.{spliceType}.tsv.gz",
		tmReads="mappings/" + "nonAnchoredMergeReads/pooled/spliceSites/{techname}_{capDesign}.introns.{spliceType}.tsv.gz",
	...
	output:  config["STATSDATADIR"] + "{techname}_{capDesign}.{spliceType}.splice.sites.stats.tsv"
	shell:
		'''
zcat {input.gencode} {input.rawReads} {input.tmReads} | sort  -k1,1 > $TMPDIR/cls.SSs.tsv
join -a1 -j1 $TMPDIR/cls.SSs.tsv {input.geneidScores} |ssv2tsv | perl -lane '$_=~s/\:/\t/g; @line=split("\\t", $_); unless(defined ($line[5])){{$line[4]=$line[2];$line[5]=-35 + rand(-25 +35)}}; print join("\\t", @line)' | awk -v t={wildcards.techname} -v c={wildcards.capDesign} '{{print t"\t"c"\t"$1"\t"$3"\t"$4"\t"$6}}'| sed 's/Corr0/\tNo/' | sed 's/Corr90/\tYes/'> {output}
...
cat {input.random} | fgrep -w {wildcards.spliceType} | awk -v t={wildcards.techname} -v c={wildcards.capDesign} '{{print t"\t"c"\t"$1"\t"$2"\trandom\t"$3}}' | sed 's/Corr0/\tNo/' | sed 's/Corr90/\tYes/' >> {output}


		'''

rule aggGeneidScores:
	input: lambda wildcards: expand(config["STATSDATADIR"] + "{techname}_{capDesign}.{spliceType}.splice.sites.stats.tsv", techname=TECHNAMES, capDesign=CAPDESIGNS, spliceType=SPLICE_SITE_TYPES)
