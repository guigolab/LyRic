rule extractPooledTmsFivepEnds:
    ...
    output:
        "output/mappings/mergedReads/5pEnds/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.5pEnds.bed",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule dhsSupportedfivepEnds:
    input:
        fivePends="output/mappings/mergedReads/5pEnds/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.5pEnds.bed",
        tms="output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-all.bed",
        dhsPeaks=lambda wildcards: GENOMETODHSPEAKS[
            CAPDESIGNTOGENOME[wildcards.capDesign]
        ],
    output:
...
rule extractPooledTmsThreepEnds:
    input:
        "output/mappings/mergedReads/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.splicing_status-all.endSupport-all.bed",
    output:
        "output/mappings/mergedReads/3pEnds/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.tmerge.min{minReadSupport}reads.3pEnds.bed",
    shell:
        """
uuidTmpOut=$(uuidgen)
...
rule getCagePolyASupport:
    ...
    output:
        ...
        + "tmp/{techname}_{capDesign}_{sizeFrac}_{sampleRep}.min{minReadSupport}reads.splicing_status-{splicedStatus}.cagePolyASupport.stats.tsv",
